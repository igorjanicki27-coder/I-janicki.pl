...
import { AlertTriangle, Cpu, HardDrive, MemoryStick, Thermometer, Workflow } from 'lucide-vue-next'
...
<template>
  <button
    type="button"
    class="group glass-panel flex w-[340px] shrink-0 flex-col rounded-[24px] border px-3.5 py-3 text-left transition duration-200 hover:-translate-y-1 sm:w-[390px] xl:w-[430px]"
  ...
  >
    <div class="flex items-start justify-between gap-3">
      <div class="min-w-0">
        <div class="flex items-center gap-2">
          ...
          <div class="display-font truncate text-sm tracking-[0.16em] text-white">{{ device.deviceAlias || device.hostname }}</div>
        </div>
    ...
    <div class="mt-2.5 grid grid-cols-4 gap-1.5">
      <div class="rounded-2xl border px-2.5 py-2" :class="metricClasses(device.telemetry?.cpuUsagePercent, props.thresholds.cpuUsage)">
        <div class="flex items-center gap-2 text-[10px] uppercase tracking-[0.18em]">
          <Cpu class="h-3.5 w-3.5" />
          CPU
        </div>
        <div class="mt-1 text-[15px] font-semibold">{{ device.telemetry?.cpuUsagePercent ?? '—' }}<span v-if="device.telemetry?.cpuUsagePercent !== null && device.telemetry?.cpuUsagePercent !== undefined">%</span></div>
      </div>

      <div class="rounded-2xl border px-2.5 py-2" :class="metricClasses(device.telemetry?.gpu?.usagePercent, props.thresholds.gpuUsage)">
        <div class="flex items-center gap-2 text-[10px] uppercase tracking-[0.18em]">
          <Workflow class="h-3.5 w-3.5" />
          GPU
        </div>
        <div class="mt-1 text-[15px] font-semibold">{{ device.telemetry?.gpu?.usagePercent ?? '—' }}<span v-if="device.telemetry?.gpu?.usagePercent !== null && device.telemetry?.gpu?.usagePercent !== undefined">%</span></div>
      </div>
    ...
    <div class="mt-1.5 grid grid-cols-3 gap-1.5">
      <div class="rounded-2xl border px-2.5 py-2" :class="metricClasses(device.telemetry?.cpuTemperatureC, props.thresholds.cpuTemp)">
        <div class="flex items-center gap-2 text-[10px] uppercase tracking-[0.18em]">
          <Thermometer class="h-3.5 w-3.5" />
          CPU Temp
        </div>
        <div class="mt-1 text-xs font-semibold">{{ device.telemetry?.cpuTemperatureC ?? '—' }}<span v-if="device.telemetry?.cpuTemperatureC !== null && device.telemetry?.cpuTemperatureC !== undefined">°C</span></div>
      </div>

      <div class="rounded-2xl border px-2.5 py-2" :class="metricClasses(device.telemetry?.gpu?.temperatureC, props.thresholds.gpuTemp)">
        <div class="flex items-center gap-2 text-[10px] uppercase tracking-[0.18em]">
          <Thermometer class="h-3.5 w-3.5" />
          GPU Temp
        </div>
        <div class="mt-1 text-xs font-semibold">{{ device.telemetry?.gpu?.temperatureC ?? '—' }}<span v-if="device.telemetry?.gpu?.temperatureC !== null && device.telemetry?.gpu?.temperatureC !== undefined">°C</span></div>
      </div>
